     ...
     
        {
          token && userData
            ...
            <div className='flex items-center gap-2 cursor-pointer group relative'>
               <span style={{ marginRight: '-30px' }}>Xin chào, {userData.name}</span>
              ...
              <div className='absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 hidden group-hover:block'>
                <div className='min-w-48 bg-gray-50 rounded flex flex-col gap-4 p-4'>
                  ...
                  <p onClick={() => navigate('/my-appointments')} className='hover:text-black cursor-pointer'>Lịch sử cuộc hẹn</p>
                  <p onClick={logout} className='hover:text-black cursor-pointer'>Đăng xuất</p>
                </div>
              </div>
            </div>
            : <button onClick={() => navigate('/login')} className='bg-primary text-white px-8 py-3 rounded-full font-light hidden md:block'>Đăng ký/ Đăng nhập</button>
        }
