...
const NewsList = () => {
  ...
  const handleAddService = () => {
    navigate('/add-news'); // Điều hướng đến trang AddService
  };

  const handleEditService = (service) => {
    navigate(`/edit-news/${service.id}`); // Điều hướng đến trang EditService với id dịch vụ
  };
      ...
      <table className="border border-[#C9D8FF] text-left text-sm lg:text-base" style={{ width: "900px", margin: "0 auto" }}>
        <thead>
          <tr className="bg-[#EAEFFF] text-left">
            <th className="p-4 border-b border-[#C9D8FF]">Tiêu đề</th>
        ...
        <tbody>
          {services.map((service, index) => (
            <tr key={index} className="hover:bg-[#F7FAFC]">
              ...
              <td className="p-4 border-b border-[#C9D8FF]">
                <Link to={`/edit-news/${service.id}`} state={service}>
