...
const EditService = () => {
  ...
  const [fees, setFees] = useState(service.price || ''); // Giá dịch vụ
  ...
  useEffect(() => {
    if (!service) {
      toast.error('Dữ liệu dịch vụ không tồn tại');
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      const { data } = await axios.put(backendUrl + '/api/admin/edit-service', formData, {
        headers: { aToken },
      });
  ...
  return (
    <form onSubmit={onSubmitHandler} className='m-5 w-full'>
      <p className='mb-3 text-lg font-medium'>Chỉnh sửa dịch vụ</p>
